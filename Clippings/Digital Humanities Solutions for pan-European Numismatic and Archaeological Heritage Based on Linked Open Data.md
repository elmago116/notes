---
authors: Oksanen, E. et al.
year: 2022
url: https://www.scopus.com/inward/record.uri?eid=2-s2.0-85139766860&partnerID=40&md5=98fa5dc27bdd0c72c905d7179221ee0b)
published in: CEUR Workshop Proceedings
title: Digital Humanities Solutions for pan - European Numismatic and Archaeological Heritage Based on Linked Open Data
tags:
  - themes/GLAM
  - design/participatory
  - Tech/KG/linked_open_data
  - tech/evaluation/KG
  - op/level/TFM-FIMA
apa_citation: Oksanen, E. et al., 2022
---
[[Digital Humanities Solutions for pan-European Numismatic and Archaeological Heritage Based on Linked Open Data.pdf]]


Authors: Oksanen, E. et al.
Year: 2022
Published in: CEUR Workshop Proceedings

Abstract: This paper discusses current challenges in archaeological cultural heritage data management and presents the interdisciplinary research project DigiNUMA. The project investigates solutions in data harmonisation and dissemination of pan-European cultural heritage through an interdisciplinary and cross-sectoral project in Digital Humanities, semantic computing, participatory heritage ( #design/participatory ==explore relationship==), museum collections management and archaeological/numismatic studies #themes/GLAM . Using Finnish and English numismatic data as a test case, DigiNUMA creates ontological infrastructure and a proof-of-concept data model for finely-grained Linked Open Data (LOD) #Tech/KG/linked_open_data harmonisation across national and international databases for cultural heritage data, and tests it through a broad suite of Digital Humanities analyses #tech/evaluation/KG #op/acc/leer #op/acc/download . © 2022 Copyright for this paper by its authors. Use permitted under Creative Commons License Attribution 4.0 International (CC BY 4.0)

Keywords:
- HerStory: archaeological and numismatic cultural heritage, museum collection management, museum, archeological and numismatic cultural heritage, cultural heritage, digital humanity, museum collection
- Knowledge Graphs: linked open data, ontology, information management, linked data, semantics, linked open datum, ontology 's
- Other: data harmonisation, open data, collection management, creative common, data harmonization



Links: [Scopus](https://www.scopus.com/inward/record.uri?eid=2-s2.0-85139766860&partnerID=40&md5=98fa5dc27bdd0c72c905d7179221ee0b)

## Extraction

- **Process-driven (phases/patterns)**:
  - “The project investigates solutions in data harmonisation and dissemination of pan-European cultural heritage through an interdisciplinary and cross-sectoral project in Digital Humanities, semantic computing, participatory heritage, museum collections management and archaeological/numismatic studies.” (Abstract)
  - “Using Finnish and English numismatic data as a test case, DigiNUMA creates ontological infrastructure and a proof-of-concept data model for finely-grained Linked Open Data (LOD) harmonisation across national and international databases for cultural heritage data…” (Abstract)
- **Methodological tools/models**:
  - Creation of ontological infrastructure and a proof-of-concept LOD data model, tested “through a broad suite of Digital Humanities analyses.” (Abstract)
- **Levels of participation/engagement**:
  - The project is framed as “participatory heritage” but this clipping does not detail concrete co-design activities; collaboration is mainly described at the level of institutions and disciplines.
- **Inclusion/representational gaps/epistemic justice**:
  - No explicit discussion of gender, intersectionality, or epistemic justice appears in this clipping; focus is on harmonising numismatic and archaeological heritage data.
## Notes for digital libraries article
- **Process-driven**:
  - “The project investigates solutions in data harmonisation and dissemination of pan-European cultural heritage through an interdisciplinary and cross-sectoral project in Digital Humanities, semantic computing, participatory heritage...” (Abstract)
  - “Using Finnish and English numismatic data as a test case, DigiNUMA creates ontological infrastructure and a proof-of-concept data model for finely-grained Linked Open Data (LOD) harmonisation across national and international databases for cultural heritage data...” (Abstract)
- **Methodological tools**:
  - “Creates ontological infrastructure and a proof-of-concept data model for finely-grained Linked Open Data (LOD) harmonisation ... and tests it through a broad suite of Digital Humanities analyses.” (Abstract)
- **Participation level**:
  - “Participatory heritage” is mentioned, but no explicit description of stakeholder participation is provided in this clipping.
- **Epistemic justice**:
  - No explicit discussion of gender, intersectionality, or bias mitigation is present in this clipping.